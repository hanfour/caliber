-- 0031_seed_platform_rubric_v2.sql
-- Seeds 3 platform rubric v2.0.0 rows (en, zh-Hant, ja) — continuous
-- facet-based scoring mirroring the ITO quarterly KPI sub-items.
-- org_id = NULL, is_default = false (pending calibration; flipping the
-- default is a separate migration per docs/RUBRIC_V2_DESIGN.md §8 step 4).
-- JSON blocks below are byte-verbatim `JSON.stringify` output of
-- packages/evaluator/src/rubrics/platformV2.ts's exported consts.
...
VALUES (
  gen_random_uuid(),
  NULL,
  'Platform Default v2 — Continuous Facet Scoring',
  ...
  "description": "Continuous scoring from LLM-judged session facets; mirrors quarterly KPI sub-items (efficiency / risk control / requester satisfaction)",
  "version": "2.0.0",
  "locale": "en",
  "scale": {
    "max": 120,
...
VALUES (
  ...
  "description": "以 LLM 逐 session 判讀的 facets 連續計分；鏡射季評分 KPI 子項（效率／風控／需求方滿意）",
  "version": "2.0.0",
  "locale": "zh-Hant",
  "scale": {
    "max": 120,
...
VALUES (
  gen_random_uuid(),
  NULL,
  'プラットフォームデフォルト v2 — 連続ファセットスコアリング',
  'LLMがセッション毎に判定したファセットによる連続スコアリング；四半期KPIサブ項目（効率／リスク管理／依頼者満足）を反映',
  '2.0.0',
  $json${
  "name": "プラットフォームデフォルト v2 — 連続ファセットスコアリング",
  "description": "LLMがセッション毎に判定したファセットによる連続スコアリング；四半期KPIサブ項目（効率／リスク管理／依頼者満足）を反映",
  "version": "2.0.0",
  "locale": "ja",
  "scale": {
    "max": 120,
